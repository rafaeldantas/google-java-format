...
/**
 * This is google-java-format, a new Java formatter that follows the Google Java Style Guide quite precisely---to the letter and to the
 * spirit.
 *
 * <p>
 * This formatter uses the Eclipse parser to generate an AST. Because the Eclipse AST loses information about the non-tokens in
 *
 *
 * the input (including newlines, comments, etc.), and even some tokens (e.g., optional commas or semicolons), this formatter lexes the
 * input again and follows along in the resulting list of tokens. Its lexer splits all multi-character operators (like ">>") into multiple
 * single-character operators. Each non-token is assigned to a token---non-tokens following a token on the same line go with that token;
 * those following go with the next token--- and there is a final EOF token to hold final comments.
 *
 * <p>
 * The formatter walks the AST to generate a Greg Nelson/Derek Oppen-style list of formatting {@link Op}s [1--2] that then generates a
 * structured {@link Doc}. Each AST node type has a visitor to emit a sequence of {@link Op}s for the node.
 *
 * <p>
 * Some data-structure operations are easier in the list of {@link Op}s, while others become easier in the {@link Doc}. The {@link Op}s are
 * walked to attach the comments. As the {@link Op}s are generated, missing input tokens are inserted and incorrect output tokens are
 * dropped, ensuring that the output matches the input even in the face of formatter errors. Finally, the formatter walks the {@link Doc} to
 * format it in the given width.
 *
 * <p>
 * This formatter also produces data structures of which tokens and comments appear where on the input, and on the output, to help output a
 * partial reformatting of a slightly edited input.
 *
 * <p>
...
public final class Formatter {
    ...
    /**
     * Construct a {@code Formatter} given a Java compilation unit. Parses the code; builds a {@link JavaInput} and the corresponding
     * {@link JavaOutput}.
     *
...
javaOutput.flush();
    ...
    /**
     * Format an input string (a Java compilation unit), for only the specified character ranges. These ranges are extended as necessary
     * (e.g., to encompass whole lines).
     *
     * @param input the input string
